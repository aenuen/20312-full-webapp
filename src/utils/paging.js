...
class Paging {
  ...
  constructor (api, pagingOptions, dataList, resKey) {
    ...
    const _pagingOpt = Object.entries(pagingOptions) // 后台分页的所需的：当前页、每页显示数量
    this.#currentObj = _pagingOpt[0]
    this.#beginPage = this.#currentObj[1]
    this.#limitObj = _pagingOpt[1]
  ...
  // 翻页，调用后台接口
  next (queryOptions, callback) {
    if (this.#isEnd) { // 如果数据已经全部加载，不再发起后台请求
      return
    }

    // 将页面中配置好的后台分页参数拼接到请求参数中去
    queryOptions[this.#currentObj[0]] = this.#currentObj[1]
    queryOptions[this.#limitObj[0]] = this.#limitObj[1]
